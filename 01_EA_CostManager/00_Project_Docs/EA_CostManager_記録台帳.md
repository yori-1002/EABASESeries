...
| 論点 | 決定 | 補足 |
| --- | --- | --- |
| つなぎの安全策 | **Phase 0 をつなぎで入れる（一時的）** | ⚠️ **読取専用は最終設計の一部ではない**。工事期間（まだ NAS 直結の間）だけの防護策で、ローカル保存方式（Phase 2）が完成したら**外す**。同時編集は「各PCがローカルDBに保存→NAS共有」から生まれるのであって、Phase 0 とは無関係（Phase 0 が有効な間は1台のみ書込可能）。yori のモデル＝この最終設計そのもの。下に具体設計 |
...
| 読取専用の強制方式（codex 指摘） | **共通書込ガード＋バナーの二重化**（接続層 read-only は Phase 1 送り） | codex の「二重化」を採用。ただし**接続層 read-only（`create_connection` を `Mode=ReadOnly`）は Phase 0 では採らない**。理由：①既存のセッションハートビートが `user_sessions` へ2分ごとに書込むため、接続を丸ごと読取専用にすると正当な書込まで壊れる ②WAL＋読取専用オープン／SMB 上の読取専用オープンの信頼性が不確実（**推測**・実機未検証）。Phase 0 の二重化＝(1) 保存メソッド側の**共通書込ガード**（読取専用フラグ時に例外/中止）＋(2) **敬語バナー**。取りこぼし経路が残りうる点は Phase 1 の書込層集約で airtight にする。接続層 read-only を採るなら業務用と heartbeat 用で接続を分ける必要があり、それは Phase 1 の範囲 |
...
| 2026-07-21 | — | `ReadOnlyGuard.cs`（新規）<br>`Views/CostPage.xaml.cs`<br>各ダイアログ6本<br>各 ViewModel 多数<br>`Services/ExcelImportService.cs`・`CostAggregationService.cs`・`EditHistoryService.cs`<br>`Views/MainWindow.xaml(.cs)` | **Phase 0 の enforcement（読取専用時に書込を止める配線）＋バナー**。①`ReadOnlyGuard.block_if_read_only()`（新規・読取専用なら敬語メッセージを出して中断）を主要なユーザー保存入口に配線（`block_if_read_only` 呼び出しは計38箇所）：日報取込（`execute_import`）・単価変更（`btn_change_rates_Click`）・再集計（`btn_reaggregate*`）・一括属性/アーカイブ・現場/絞り込みメニュー（書込分岐のみ／`show_archived` 等の閲覧は許可）・分類設定/キーワード/現場単価/現場編集/現場登録/絞り込み作成の各ダイアログ・タブ操作VM（複製/削除/アーカイブ/復元/モード適用）・現場並べ替え・管理系VM（社員/車両/機材/名寄せ/権限/NAS設定/区分結合の save/toggle/delete/add）・完全削除/復元 ②**サービス層 hard backstop**（UI取りこぼしの二重化）：`ExcelImportService.ImportAsync` 先頭に `ensure_writable()`、`CostAggregationService.aggregate_async` は読取専用時にエラー文字列を返す、`EditHistoryService.undo/redo_async` は読取専用時 -1 ③**折りたたみ状態の保存**（`workload_view_model`/`filter_tab_view_model`）は読取専用時スキップ（画面の開閉は可能・NASへ書かないだけ） ④`MainWindow` 最上部に読取専用の赤帯バナー（起動時に `UserSession.is_read_only` から表示）。**除外**：セッションのハートビート（`user_sessions` 更新）・初回ユーザー登録（`FirstLoginDialog`）・起動時のPC名/セッション初期化＝読取専用でも動かす。ビルド0エラー・警告35（増減なし）。**未ガードとして残しうる長い尾は Phase 1 の書込層集約で airtight 化**。**画面での動作確認は未実施**。 | 追加 |
| 2026-07-20 | — | `Data/NasWriteLock.cs`（新規）<br>`UserSession.cs`<br>`Data/database_manager.cs`<br>`App.xaml.cs`<br>`ViewModels/main_view_model.cs`<br>`Views/MainWindow.xaml.cs` | **Phase 0（つなぎの安全策）の中核を実装＝NAS 書込ロック＋読取専用フラグ＋共通書込ガード**。①`NasWriteLock`（NAS 上のロックファイル `<db>.lock`・`.tmp`→rename で原子的更新・5分陳腐化基準・自分の残骸は引き継ぎ・失敗時はフェイルセーフで書込可能）②`UserSession.is_read_only` / `read_only_reason` / `set_read_only` を追加 ③`database_manager.ensure_writable()` ＋ `ReadOnlyModeException`（業務保存の入口で呼ぶ共通書込ガード。**接続層 read-only は採らない**＝セッションのハートビート書込を壊さないため。決定事項表参照）④起動フロー（`App.xaml.cs` の NAS 有効ブロック末尾）でロック取得を試み、取れなければ読取専用フラグを立てる ⑤既存の2分 `DispatcherTimer`（`main_view_model`）で heartbeat 更新 ⑥終了時（`MainWindow` finally）で release（保持中のみ・読取専用PCは no-op）。**検証**：実コードのロジックを移植した検証プログラムで9項目パス（空取得・他PC読取専用・保持者名・自分の残骸引き継ぎ・release消滅・陳腐化奪取・生存は奪取しない・読取専用PCのreleaseは他PCロックを消さない・.tmp残らない）。ビルド0エラー。**未実装（次スライド）**：各保存メソッドから `ensure_writable()` を呼ぶ配線＋読取専用バナー表示。**画面での動作確認は未実施**。 | 追加 |
