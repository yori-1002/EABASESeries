...
namespace EA_DailyReport.Services
{
    ...
    ///   2. NAS の 02_Updates\ に新インストーラー（EA_DailyReport_setup_vX.X.X.exe）を置く
    ...
    public static class UpdateCheckerService
    {
            ...
            CancellationToken ct = default)
        {
            try
            {
                // NAS上のインストーラーファイル名（例: EA_DailyReport_setup_v1.1.0.exe）
                ...
                string temp_dir = Path.Combine(Path.GetTempPath(), "EA_DailyReport_update");
